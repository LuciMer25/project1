        ...
        export default {
            ...
            methods: {
                async getCategories(topCtgrNo, ctgrNo) {
                    try {
                        ...
                        if (result.data.length > 0) {
                            // 첫 번째 상위 카테고리의 이름 설정
                            this.selectedTopCategory = result
                                .data[0]
                                .top_ctgr_name;
                            ...
                            this.subCategories = [...subCategoriesSet].map(category => ({
                                ...
                                ctgr_no: result
                                    .data
                                    .find(item => item.ctgr_name === category)
                                    .ctgr_no
                            }));

                            // 전달받은 ctgrNo에 해당하는 하위 카테고리를 selectedSubCategory로 설정
                            this.selectedSubCategory = this
                                .subCategories
                                .find(sub => sub.ctgr_no === Number(ctgrNo)) || null;
                        }
                    } catch (error) {
                ...
                handleSubCategoryClick(subcategory) {
                    ...
                    this
                        .$router
                        .push(`/category/${this.topCtgrNo}/${subcategory.ctgr_no}`);
                },
                handleTotal() {
