<template>
    <v-container fluid>
      <v-breadcrumbs class="mb-4">
        <v-breadcrumbs-item>취소 현황</v-breadcrumbs-item>
      </v-breadcrumbs>
      <v-card class="mb-4">
        <v-card-title>
        ...
        <v-card-text>
          ...
          >
            ...
            <template v-slot:header.cancel_req_date>
              <th class="text-left">주문취소일자</th>
            </template>
            ...
            <template v-slot:item.order_state="{ item }">
              <span
                v-if="item.order_state === '취소요청'"
                class="cancelSpan"
                @click="cancelBtn(item.order_no)"
              >
                {{ item.order_state }}
              </span>
              <span v-else>{{ item.order_state }}</span>
            </template>
            <template v-slot:item.cancel_req_date="{ item }">
              {{ formatDate(item.cancel_req_date) }}
            </template>
          </v-data-table>
        </v-card-text>
      </v-card>
  ...
  export default {
    data() {
      return {
        ...
        headers: [
          ...
          { text: 'cancel_req_date', value: 'cancel_req_date' },
    ...
    computed: {
      filteredItems() {
        return this.cancelAllList.filter((item) => {
    ...
    methods: {
      async fetchCancelList() {
        try {
          const res = await axios.get('/api/adminOrder/cancelALLList');
